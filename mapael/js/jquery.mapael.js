	...
	*/
	$.fn.mapael.initZoom = function($container, paper, mapWidth, mapHeight, options) {
		...
		$container.bind("touchstart", function(e) {
			mousedown = true;
			previousX = e.originalEvent.changedTouches[0].pageX;
			previousY = e.originalEvent.changedTouches[0].pageY;
		}).bind('touchmove',function(e){
			//console.log(e.originalEvent.changedTouches[0].pageX);
			var currentLevel = $parentContainer.data("zoomLevel");
			if (mousedown && currentLevel != 0) {
				var offsetX = (previousX - e.originalEvent.changedTouches[0].pageX) / (1 + (currentLevel * options.step)) * (mapWidth / paper.width)
					, offsetY = (previousY - e.originalEvent.changedTouches[0].pageY) / (1 + (currentLevel * options.step)) * (mapHeight / paper.height)
					, panX = Math.min(Math.max(0, paper._viewBox[0] + offsetX), (mapWidth - paper._viewBox[2]))
					, panY = Math.min(Math.max(0, paper._viewBox[1] + offsetY), (mapHeight - paper._viewBox[3]));					
