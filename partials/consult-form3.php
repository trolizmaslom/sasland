<div class="consult-from3-wrap">
	<div class="consult-form3 container">
		<div class="form-wrap row">
			<div class="col-sm-8">
				<h2>Оставьте заявку на консультацию</h2>
			</div>
			<div class="col-sm-6">
				<p>Мы расскажем о рисках и ответственности Вашего  бизнеса для СЭЗ в Крыму</p>
			</div>
			<div class="col-sm-6">
				<form id="consult-form3" method="POST" action="" class="ajax-form">
					<div class="input-wrap col-sm-6 phone">
						<input type="text" name="phone" placeholder="Ваш телефон" required="required"></input>
					</div>
					<div class="input-submit-wrap col-sm-6">
						<button type="submit">Записаться на консультацию</button>
					</div>
					<div class="col-sm-12 cfix">
						<p>Мы гарантируем конфиденциальность ваших данных</p>
					</div>
				</form>
			</div>
		</div>
	</div>
</div>